...
Because of its functional design, _ClojureScript_ typically favors recursion over
imperative, statement based loops. Recursion sometimes requires additional thinking about
how to model your problem in a different way than imperative languages.

Many of the common patterns which use `for` in other languages are achieved through
...
In the above snippet, we bind the name `x` to the value `0` and execute the body. Since
the condition is not met the first time it is run we `recur`, incrementing the binding
value with the `inc` function. We repeat this once more until the condition is met and
since there are no more `recur` calls, exit the loop.

Note that `loop` isn't the only point we can `recur`, using `recur` inside a function
...
In imperative programming languages it is common to use `for` loops to iterate over data
and transform it, where the intent is usually one of the following:

- Transform every value in the iterable, yielding another iterable
- Filter the elements of the iterable by some criteria
