...
transformatios, so if you have nested objects, the nested objects will not be converted.
To solve that use cases _ClojureScript_ comes with the `clj->js` and `js->clj` functions
that transform clojure collection types into JavaScript and bacl:
...
collection. But it there other function for creating arrays: `make-array`.
