...
associated with the current namespace. For convenience, _ClojureScript_ also defines a constructor function caled
...
imposibility to redefine vars from other namespaces is a bit limiting; also, if we are modifying state we're
...
will. Besides from altering its value, it also supports observation through watcher functions that can
be attached and dettached from it and validation for ensuring that the value contained in the atom is
always valid.
...
watcher has key associated (`:logger` in the example) to it, which is later used to remove the watch from the atom.
