...
Many of the ClojureScript core functions that work on collections call `seq` on their arguments, thus being
implemented in terms of generic sequence operations.
...
Protocols provides a type based polymorphism, and the dispatch is always done by the first argument
previously mentioned as `this`.
...
(defprotocol IUser
  ...
  (full-name [x] "Get a full name of the user."))
----
...
NOTE: Inside the clojure community the convention for protocol functions is treat them as private api
and not expose them to the general public api.
