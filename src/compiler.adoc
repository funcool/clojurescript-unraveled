...
available (such as http://leiningen.org/[leiningen]+https://github.com/emezeske/lein-cljsbuild[cljsbuld]
or http://boot-clj.com/[boot]).
...
Surelly you have noticed that repl does not have support for history and other shell like facilities, that
is happens because the default REPL does not comes with "readline" support. But it can be solved using
the simple tool called `rlwrap` that you can found surelly on the package manager of your operating system.
...
TBD
>>>>>>> d72c09b35074df9e2027d5860bd4319220039bd5
...
as execution environment and it has additional requirements.
...
REPL working. For it, just recreate the application structure very similar to that one
