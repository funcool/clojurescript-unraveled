...
available (such as http://leiningen.org/[leiningen] + https://github.com/emezeske/lein-cljsbuild[cljsbuild]
...
followed by the second most popular - https://nodejs.org/[nodejs]/https://iojs.org/en/index.html[iojs].
...
is loaded (the page will be blank), switch back to the console where we have run the REPL
