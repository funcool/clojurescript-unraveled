...
management tool to build _ClojureScript_ projects. There is another build tool growing in popularity
called *boot*, but at this time, it will not to be covered in this book.
...
line of export to your `~/.bashrc` file (we are supposing that you are using the bash shell).
...
NOTE: We are supposing that you are using a Unix-like system such as Linux or BSD. If you are a
