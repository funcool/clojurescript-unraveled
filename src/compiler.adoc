...
The best way to show how a tool works, is creating an toy project with it. In this case we will
go to create one little application that calculates if a year is leap or not. And for it, we will
use the *mies* leiningen template.

NOTE: Templates are a way that leiningen has to provide a initial layour for an application
or a library and clojure community has a great amount of them. In this case we'll use *mies*
template that is maintained by the clojurescript core developer. If you want to know more about
templates, you should read the leiningen doc. Explain templates is completely out of scope of
this book.
...
code into the `stc/leapyear/core.cljs`:
...
Additionally to `watch` script, the *mies* template also generates a bunch
of other scripts such as `build`,`release`,... but for our purposes on this section
them are completely useless. And probably after reading the compiler and repl
sections you already will be familiarized with almost all scripts that comes
with *mies* template.
...
dependencies. Because it makes this task more obvios in comparison if we should do
it manually.

The dependencies, among other parameters are declared in the `project.clj` file
and has this aspect (from *mies* template):
...
from clojure or any JVM language, the `jar` files will be very familiar for you. But
if you don't familiar with them, do not worry: a jar file is like a plain zip file
that contains the `project.clj` of the library, some metadata and the clojurescript
sources. The packaging will be explained in other section.

The clojure packages are often published in the link:http://clojars.org[Clojars],
also you can found a bunch of different third party libraries on the
