...
Threading macros, also kwno as arrow functions, enables write more readable code
when multiple neested function calls is performed.

Imagine you have `(f (g (h x)))` where a function `f` receives as first parameter a
result of executing of function `g` and the same repeated multiple times. With the
most basic `->` threading macro you can convert that in `(\-> x (h) (g) (f))` that is
much more easy to read.

The result is just a sugar syntax because the arrow functions are defined as macros
and it does not imply any runtime performance. The `(\-> x (h) (g) (f))` is
automatically converted to (f (g (h x))) at compile time.

Take care that the parenthesis on `h`, `g` and `f` are optional, and can be ommited:
`(\-> x h g f)`.


==== The thread-first macro (`\->`)

Is called *thread first* becaiuse it threads the first argument throught the
diffenrent expressions as first arguments.

Let see more concrete example, this is how the code looks without using threading
macros:
...
We can rewrite that code to use `\->` threading macro:
...
This threading macro is specially useful for transforming data structures, bacuase
...
The main difference of this macro to the previous, is that instead of threading
the first argument as first argument on the following expresions, it threads
...
This threading macro is specially useful for transforming sequences or collections
of data because _ClojureScript_ functions that works with sequences and collections
consistently uses the last argument position for receive them.


==== The thread-as macro (`as\->`)

Finally, there are cases where neither `\->` nor `\->>` are applicable. In these
cases, you’ll need to use `as\->`, the more flexible alternative, that allows thread
into any argument position, not just first or last.

It expects two fixed arguments and a arbitrary number of expressions. As with
...
Two of more specialized threading macros that _ClojureScript_ comes with. They works
in the same way as their analgous `\->` and `\->>` macros with the additional
...
To use reader conditionals, all you need is you need to rename your source file with
`.cljs` extension to one with `.cljc`, because reader conditionals only works if
them are placed in files with `.cljc` extension.
...
Yoy may note that a special symbol *string* is used instead of `js/String` for
extend the protol for string. This is because the builtin javascript types have
special treatmen and if you replace the `string` with `js/String` the compiler
