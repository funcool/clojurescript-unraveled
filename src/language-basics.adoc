...
`+` is not an operator but a function, Lisp has no operators, only functions.
...
If a function only accepts one argument, you can omit the number after the `%` symbol, e.g., the
...
The `case` branching expression has a similar use case to our previous example with
...
////
Joe the Beginning Programmer asks: "What do you mean by 'attaching metadata to vars'? I'm confused."
////
...
returned by the built-in `compare` function. Comparator functions take two elements to compare and return -1 (less than), 0 (equal)
or 1 (greater than):
...
Not only keywords can be the keys of associative data structures. Numbers, strings, symbols and
many other data structures can be used as keys, so we can destructure using those, too. Note that
we need to quote the symbols to prevent them from being resolved as a var lookup:
...
will use a *set*. See this snipped of code:
...
We can add and remove watcher functions to atoms. Whenever the atom's value is changed through a `swap!` or `reset!`,
