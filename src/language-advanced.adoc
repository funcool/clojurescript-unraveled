...
and calling the reducing function with accumulator, key and value as positional arguments:
...
There are some values that have the notion of asynchronous computation, the value they represent may not
...
When we deref a delay the computation is run and the delay is realized:
...
We know that `deref` is based on the `IDeref` protocol. `reset!` is based on the `IReset` protocol and `swap!` on
...
Now let's check that we got it right. We'll add a watch, change the atom's value and remove the watch making sure the
watch gets called:
...
structure: `read-string`. It's memoized for not having to call the reader each time we deserialize string since
