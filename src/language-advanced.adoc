...
Note that transducible processes must respect `reduced` as a way for signaling early termination. For example,
...
==== More transducers from ClojureScript core
...
reducing function in arities 0 and 1, there is a little helper called `completion` that fills those arities
for you:
