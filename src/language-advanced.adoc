...
several purposes like type annotations and the metadata system can be used bt tool and application developers
too.

==== Vars

Let's define a var and see what metadata is attached to it by default, note that this code is executed in
a REPL and thus the metadata of a var defined in a namespace may vary. We'll use the `meta` function to
...
==== What can I use metadata for?

We have seen that metadata is useful to convey relevant information to the compiler such as type hints and for
usage in libraries like `cljs.test`. Metadata can also be used for runtime code introspection or documentation
generation, for example. There may not be many cases in day-to-day ClojureScript programming where you need
metadata but is a nice language feature to have and know about, it may come in handy sometime, specially for
tooling and library authors.
