...
ClojureScript's main abstractions. For learning about the sequence procols we'll define
a `Pair` type, which will hold a pair of values.
...
(deftype Pair [fst snd]
  ...
  (-pr-writer [o writer _]
    (-write writer (str "#<Pair " fst "," snd ">"))))
----

///
TODO: Still haven't mentioned IPrintWithWriter, maybe more Pair definition to section
start and see how we can make it printable as the first example of the core protocols.
////

Remember the `first` and `rest` functions for working with sequences? They are defined in
the `ISeq` protocol, so we can extend types for responding to such functions:
